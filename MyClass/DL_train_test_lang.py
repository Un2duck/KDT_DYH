# ---------------------------------------------------------------------
# Version.1
# file_name : DL_train_test_lang.py
# Date : 2024-09-19
# 설명 : wikitest.csv train val test 진행
# 변경사항 : 모델 저장 추가
# ---------------------------------------------------------------------
...
DATA_FILE='./Data/wikitest.csv'

# 저장 경로
SAVE_PATH='../Models/lang/'

# 저장 파일명
SAVE_FILE=SAVE_PATH+'lang_reg_model_train_wbs.pth'

# 모델 구조 및 파라미터 모두 저장 파일명
SAVE_MODEL=SAVE_PATH+'lang_reg_model_all.pth'
...
# 모든 열에 대해 소수점 4자리로 반올림
dataDF = dataDF.apply(lambda x: x.round(4) if x.dtype == 'float64' or x.dtype == 'float32' else x)

# ---------------------------------------------------------------------
# 데이터셋 인스턴스 생성
# DataFrame에서 피쳐와 타겟 추출
featureDF = dataDF.iloc[:,1:] # 2D
targetDF = dataDF.iloc[:,-1] # 2D

# 피쳐 인코딩 진행
targetDF = convert_label(targetDF)
